import {
    dif, Pi, polGip, polIzmEnMeh, polIzmEnMeh1, polJpMeh, polJpZv, polKompSumVek, polMax, polMdvMeh, polMpF,
    polMpMeh, polMpMehMass, polPlecho, polUg2Vek, polUgGrad, polVes, polVesVek
} from "../utils.js";
import {
    polTokAB, polTokAS2, polTokBC, polTokBD, polTokBS3, polTokBS4, polTokOA, polTokTakt,
    polTokW, polTokWDv, polTokA, polTokB, polTokM3, polTokM1, polTokM2, polTokM4, polTokM5,
    polTokJs3, polTokJs2, polTokJs4, polTokJsDv, polTokP5Max, polTokJs1, polTokK, polTokToch, polTokFi0
...
function polFIAC(t) {
    return Math.acos((polTokA() - polSAX(t)) / polAC(t));
}
...
export function polSS3X(t) {
    return polTokA() + (polTokBC() - polTokBS3()) * Math.cos(polFI3(t));
}

export function polSS3Y(t) {
    return polTokB() + (polTokBC() - polTokBS3()) * Math.sin(polFI3(t));
}

export function polFI4(t) {
    return Math.asin((polSBY(t) - polTokB()) / polTokBD()) + Pi;
}
...
function polOC() {
    return polGip(polTokA(), polTokB());
}

function polFiOC() {
    return Math.atan(polTokB() / polTokA());
}

export function polLevKr() {
    return polTokA() - polTokBC() - polTokBD();
}

export function polVerhKr() {
    return (polTokOA() + polTokAB()) * Math.sin(polFiOC() + polUgPo3St(polTokBC(), polTokOA() + polTokAB(), polOC()));
}
...
export function polPrav5(verh) {
    const A = polTokA(), OA = polTokOA(), AB = polTokAB(), BC = polTokBC(), BD = polTokBD();
    let OB;
    verh ? OB = (AB + OA) : OB = (AB - OA);
    const fi3 = Math.abs(polUgPo3St(OB, BC, polOC()) - polFiOC());
    return A - BD * Math.sin(Pi - fi3 - Math.asin(Math.sin(fi3) * BC / BD)) / Math.sin(fi3);
}

export function polDiap5(verh) {
    return polPrav5(verh) - polLev5();
}

export function polPrav5Kr() {
    return polMax([polPrav5(), polPrav5(true)]);
}
...
export function polP5(t) {
    const lev = polLev5();
    const S = polSDX(t);
    const diapN = polDiap5();
    const diapV = polDiap5(true);
    const P = polTokP5Max();
    if (polSBY(t) <= polTokB() && polVDX(t) <= 0) {
        if (lev + 0.3 * diapN <= S && S <= lev + 0.4 * diapN) {
            return P * (0.3 * (S - lev - 0.3 * diapN) / (0.1 * diapN));
        } else if (lev + 0.4 * diapN < S && S <= lev + 0.5 * diapN) {
            return P * (0.3 - 0.3 * (S - lev - 0.4 * diapN) / (0.1 * diapN));
        } else if (lev + 0.7 * diapN <= S && S <= lev + 0.95 * diapN) {
            return P * ((S - lev - 0.7 * diapN) / (0.25 * diapN));
        } else if (lev + 0.95 * diapN < S) {
            return P * (1 - (S - lev - 0.95 * diapN) / (0.05 * diapN));
        } else {
            return 0;
        }
    } else if (polVDX(t) < 0 && lev + 0.5 * diapV < S) {
        return 0.2 * P;
    } else {
        return 0;
    }
...
export function polPlA(t) {
    return (polSBX(t) - polSDX(t)) / 1000;
}

export function polPlB(t) {
    return Math.abs(polSBY(t) - polSDY(t)) / 1000;
}
...
export function polJp(t) {
    return polJpMeh([
        [polTokM2(), polVS2(t)], [polTokM3(), polVS3(t)], [polTokM4(), polVS4(t)], [polTokM5(), polVD(t)],
        [polTokJs1(), polW1(t)], [polTokJs2(), polW2(t)], [polTokJs3(), polW3(t)], [polTokJs4(), polW4(t)],
        [polTokJsDv(), polWDv(t)]
    ], polW1(t));
}
...
export function polMpG3(t) {
    return polMpF(polG3(true), polVS3Y(t), Math.abs(polW1()));
}
...
export function polMpP5(t) {
    return polMpF(polP5(t), polVDX(t), Math.abs(polW1()));
}
