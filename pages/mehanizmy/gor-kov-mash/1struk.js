...
import {
    polFI1, polFI2, polFI4, polLevKr, polNizhKr, polPravKr, polSAX,
    polSAY, polSBX, polSDY, polSS2X, polSS2Y, polSS4X, polSS4Y, polVerhKr
} from "./analiz.js";
import {
    polTekVr,
    polTokA, polTokAB, polTokBD, polTokBS4, polTokK, polTokOA, polTokTakt, polTokToch
...
const mehRis = elNaStr("meh_ris"), st1 = elNaStr("st_1"), st3 = elNaStr("st_3"), st5 = elNaStr("st_5"),
    ...
    zv2 = elNaStr("zv_AB"), zv3 = elNaStr("pol3"), zv4 = elNaStr("zv_BD"), zv5 = elNaStr("pol5"), s2 = elNaStr("S2"),
...
const OA = polTokOA(), AB = polTokAB(), BD = polTokBD(), BS4 = polTokBS4();
...
let planPolTag =
    ...
    polTagOpSk(polTokOA() + 10, null, null, AB + 20, false, false, null, null) +
    polTagOpSk(polTokA(), BS4 + 10, 90, BD, false, true, null, null);
for (let i = 0; i < KOL; i++) {
    const t = i * TAKT / KOL;
    planPolTag +=
        ...
        polTagZvNeissl(polSBX(t), null, polUgGrad(polFI4(t)), BD, 11) +
...
planPolTag +=
    ...
    polTagZvAkt(polSBX(T), null, polUgGrad(polFI4(T)), BD, 11) +
