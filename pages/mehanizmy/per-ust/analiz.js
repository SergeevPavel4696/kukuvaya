import {
    dif, Pi, polGip, polIzmEnMeh, polIzmEnMeh1, polJpMeh, polJpZv, polMdvMeh, polMpF, polMpMeh,
    polMpMehMass, polPlecho, polUg2Vek, polUgGrad, polVes, polVesVek
} from "../utils.js";
import {
    polTokOA, polTokAB, polTokAS2, polTokW, polTokTakt, polTokA, polTokCD, polTokCS4,
    polTokAC, polTokM1, polTokM2, polTokM3, polTokM4, polTokM5, polTokJs1, polTokJs2,
    polTokJs4, polTokJsDv, polTokP5Max, polTokP3Max, polTokWDv, polTokK, polTokToch
...
export function polFI4(t) {
    return -Math.acos((polTokA() - polSCX(t)) / polTokCD(t));
}
...
export function polNizhKr() {
    return -polTokOA() * polTokAC() / polTokAB() - polTokCD();
}

export function polLev3() {
    return polTokAB() - polTokOA();
}
...
export function polVerh5() {
    return polTokOA() * polTokAC() / polTokAB();
}
...
export function polP3(t) {
    ...
    if (v >= 0) {
        if (S <= min + 0.5 * diap) {
            return -P * (S - min) / (0.5 * diap);
        } else if (min + 0.5 * diap < S && S <= min + 0.75 * diap) {
            return -P;
        } else {
            return -P * (1 - (S - min - 0.75 * diap) / (0.25 * diap));
        }
    } else {
        return 0;
    }
...
export function polMomR05(t) {
    return polR05(t) * (polSCY(t) - polSDY(t)) / 1000;
}

export function polMomP5(t) {
    return -polP5(t) * (polSCX(t) - polSDX(t)) / 1000;
}

export function polMomFIn5(t) {
    return -polFIn5Y(t) * (polSCX(t) - polSDX(t)) / 1000;
}
...
export function polPlB(t) {
    return Math.abs(polSCX(t) - polSDX(t)) / 1000;
}
...
export function polR03(t) {
    return (polMomR42(t) + polMomP3(t) + polMomG3(t) + polMomFIn3(t) + polMomG2(t) + polMomFIn2(t) + polMIn2(t)) /
...
export function polR12X(t) {
    return -polR32X(t) - polFIn2X(t) - polR42X(t);
}

export function polR12Y(t) {
    return -polR32Y(t) - polFIn2Y(t) - polG2(true) - polR42Y(t) ;
}
