import {
    elFocusByPole, massFocus, momInFocus, poleFocusByEl, polUgGrad, zadPole, zadPerVHran, Pi, polT, polUglSk
} from "../utils.js";
import {
    polFI1, polFI2, polFI4, polLevKr, polNizhKr, polPravKr, polVerhKr, polSAX,
    polSAY, polSBX, polSBY, polSS2X, polSS2Y, polSS4X, polSS4Y, polSCX, polSCY, polNizh3, polNizh5, polSB, polSC
} from "./analiz.js";
import {
    polTokOA, polTokAB, polTokAC, polTokFi, polTokK, polTokM1, polTokM2, polTokM3, polTokM4,
    polTokM5, polTokJs1, polTokJs2, polTokJs4, polTokN1, polTokP3Max, polTokP5Max, polTokToch,
    tokOA, tokAB, tokAC, tokFi, tokM1, tokM2, tokM3, tokM4, tokM5, tokJS1, tokJS2, tokJS4, tokK,
    tokN1, tokToch, polTokNerav, tokNerav, polTokAS2, polTokAS4, polTokJsVozd, tokAS2, tokAS4,
    tokP3Max, tokP5Max, tokJSVozd, polTokNVozd, tokNVozd, polTokD3, polTokD5, tokD3, tokD5
...
contentTag += pol0par([
    ...
    ["nvozd_vvod", "n<sub>в</sub>, об/мин", 0],
    ...
    ["jsvozd_vvod", "J<sub>возд</sub>, кг×м<sup>2</sup>", 0],
...
const
    ...
    ac_vvod = elNaStr("ac_vvod"), as4_vvod = elNaStr("as4_vvod"), d3_vvod = elNaStr("d3_vvod"),
    d5_vvod = elNaStr("d5_vvod"), fi_vvod = elNaStr("fi_vvod"), n1_vvod = elNaStr("n1_vvod"),
    nvozd_vvod = elNaStr("nvozd_vvod"), m1_vvod = elNaStr("m1_vvod"), m2_vvod = elNaStr("m2_vvod"),
    ...
    jsvozd_vvod = elNaStr("jsvozd_vvod"), p3_vvod = elNaStr("p3_vvod"), p5_vvod = elNaStr("p5_vvod"),
...
const
    ...
    shA = elNaStr("shA_ris"), shB = elNaStr("shB_ris"), shC = elNaStr("shC_ris"), s2 = elNaStr("s2_ris"),
...
function zadRazmIzobr() {
    const t = polTekVr();
    zadRisRazm(meh, meh1, 69.3 + (polPravKr() - polLevKr()), 41.65 + (polVerhKr() - polNizhKr()), 34.65 - polLevKr(), 34.65 + polVerhKr());
    zadPerVrEl(st3, (polNizh3() - 90) * Math.cos(0.75 * Pi), (polNizh3() - 90) * Math.sin(0.75 * Pi), 135);
    zadPerVrEl(st5, (polNizh5() - 90) * Math.cos(0.25 * Pi), (polNizh5() - 90) * Math.sin(0.25 * Pi), 45);
    zadDlZv(tr3, polSB(t) - polNizh3() + 90, 4);
    zadDlZv(tr5, polSC(t) - polNizh5() + 90, 4);
}
...
function zadZv3Pol() {
    const t = polTekVr();
    zadPerVrEl(zv3, polSBX(t), polSBY(t), 135);
}
...
function zadZv5Pol() {
    const t = polTekVr();
    zadPerVrEl(zv5, polSCX(t), polSCY(t), 45);
}
...
zadPole(nvozd_vvod, polTokNVozd());
...
zadPole(jsvozd_vvod, polTokJsVozd());
...
nvozd_vvod.addEventListener("input", function () {
    zadPerVHran(tokNVozd, nvozd_vvod.value);
});
...
jsvozd_vvod.addEventListener("input", function () {
    zadPerVHran(tokJSVozd, jsvozd_vvod.value);
});
