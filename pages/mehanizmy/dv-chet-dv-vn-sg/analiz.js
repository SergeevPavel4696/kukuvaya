...
import {
    polTokOA, polTokAB, polTokAS2, polTokOC, polTokCD, polTokCS4, polTokW,polTokTakt,
    polTokM1, polTokM2, polTokM3, polTokM4, polTokM5, polTokJs1, polTokJs2, polTokJs4,
    polTokD3, polTokP3Max, polTokP5Max, polTokD5, polTokK, polTokToch
...
export function polFI2(t) {
    return Math.asin(polSAY(t) / polTokAB()) + Pi;
}
...
export function polFI4(t) {
    return Math.asin(-polSCY(t) / polTokCD());
}
...
export function polP(vVek, Smin, SVek, diap, P, t, takt, ug) {
    ...
    if((t / takt) % 2 === 0) {
        ...
        return 0.29 * P;
    } else if (Math.floor(t / takt) % 2 === 0) {
        if (v <= 0) {
            if (S <= min + 0.95 * diap) {
                return -0.01 * P;
            } else {
                return P * (-0.01 + 0.02 * (S - min - 0.95 * diap) / (0.05 * diap));
            }
        } else {
            if (S <= min + 0.2 * diap) {
                return P * (-0.01 + 0.01 * (S - min) / (0.2 * diap));
            } else if (min + 0.2 * diap <= S && S <= min + 0.4 * diap) {
                return P * (0.014 * (S - min - 0.2 * diap) / (0.2 * diap));
            } else if (min + 0.4 * diap <= S && S <= min + 0.5 * diap) {
                return P * (0.014 + 0.016 * (S - min - 0.4 * diap) / (0.1 * diap));
            } else if (min + 0.5 * diap <= S && S <= min + 0.6 * diap) {
                return P * (0.03 + 0.01 * (S - min - 0.5 * diap) / (0.1 * diap));
            } else if (min + 0.6 * diap <= S && S <= min + 0.7 * diap) {
                return P * (0.04 + 0.02 * (S - min - 0.6 * diap) / (0.1 * diap));
            } else if (min + 0.7 * diap <= S && S <= min + 0.8 * diap) {
                return P * (0.06 + 0.04 * (S - min - 0.7 * diap) / (0.1 * diap));
            } else if (min + 0.8 * diap <= S && S <= min + 0.9 * diap) {
                return P * (0.1 + 0.06 * (S - min - 0.8 * diap) / (0.1 * diap));
            } else if (min + 0.9 * diap <= S && S <= min + 0.95 * diap) {
                return P * (0.16 + 0.04 * (S - min - 0.9 * diap) / (0.05 * diap));
            } else if (min + 0.95 * diap <= S && S <= min + 0.975 * diap) {
                return P * (0.2 + 0.03 * (S - min - 0.95 * diap) / (0.025 * diap));
            } else {
                return P * (0.23 + 0.06 * (S - min - 0.975 * diap) / (0.025 * diap));
            }
        }
    } else {
        if (v <= 0) {
            if (S <= min + 0.1 * diap) {
                return P * (0.05 + 0.07 * (S - min) / (0.1 * diap));
            } else if (min + 0.1 * diap <= S && S <= min + 0.2 * diap) {
                return P * (0.12 + 0.02 * (S - min - 0.1 * diap) / (0.1 * diap));
            } else if (min + 0.2 * diap <= S && S <= min + 0.3 * diap) {
                return P * (0.14 + 0.03 * (S - min - 0.2 * diap) / (0.1 * diap));
            } else if (min + 0.3 * diap <= S && S <= min + 0.4 * diap) {
                return P * (0.17 + 0.02 * (S - min - 0.3 * diap) / (0.1 * diap));
            } else if (min + 0.4 * diap <= S && S <= min + 0.6 * diap) {
                return P * (0.19 + 0.1 * (S - min - 0.4 * diap) / (0.2 * diap));
            } else if (min + 0.6 * diap <= S && S <= min + 0.7 * diap) {
                return P * (0.29 + 0.07 * (S - min - 0.6 * diap) / (0.1 * diap));
            } else if (min + 0.7 * diap <= S && S <= min + 0.8 * diap) {
                return P * (0.36 + 0.14 * (S - min - 0.7 * diap) / (0.1 * diap));
            } else if (min + 0.8 * diap <= S && S <= min + 0.9 * diap) {
                return P * (0.5 + 0.2 * (S - min - 0.8 * diap) / (0.1 * diap));
            } else if (min + 0.9 * diap <= S && S <= min + 0.975 * diap) {
                return P * (0.7 + 0.3 * (S - min - 0.9 * diap) / (0.075 * diap));
            } else {
                return P * (1 - 0.71 * (S - min - 0.975 * diap) / (0.025 * diap));
            }
        } else {
            if (S <= min + 0.1 * diap) {
                return P * (0.05 - 0.04 * (S - min) / (0.1 * diap));
            } else {
                return 0.01 * P;
            }
        }
...
export function polP3(t) {
    return polP(polVBX(t), polPrav3(), polSBX(t), polDiap3(), polP3Max(), t, polTokTakt(), Pi);
}

export function polP5(t) {
    return polP(polVDX(t), polLev5(), polSDX(t), polDiap5(), -polP5Max(), t + polTokTakt(), polTokTakt(), 0);
}

//Вторая группа Ассура.
export function polMomP5(t) {
    return polP5(t) * polSCY(t) / 1000;
}

export function polMomFIn5(t) {
    return polFIn5X(t) * polSCY(t) / 1000;
}
...
export function polPlD(t) {
    return Math.abs(polSS4X(t) - polSCX(t)) / 1000;
}
...
export function polMomP3(t) {
    return polP3(t) * polSAY(t) / 1000;
}

export function polMomFIn3(t) {
    return polFIn3X(t) * polSAY(t) / 1000;
}
...
export function polMomR03(t) {
    return polR03(t) * (polSBX(t) - polSAX(t)) / 1000;
}
...
export function polR23X(t) {
    return -polP3(t) - polFIn3X(t);
}

export function polR23Y(t) {
    return -polR03(t) - polG3(true);
}
...
export function polUgP3VB(t) {
    return (polVBX(t) * polP3(t) >= 0 ? 0 : 180);
}

export function polUgP5VD(t) {
    return (polVDX(t) * polP5(t) >= 0 ? 0 : 180);
}
...
export function polMpP3(t) {
    return polMpF(polP3(t), polVBX(t), Math.abs(polW1(t)));
}
...
export function polMp(t) {
    return polMpMeh([[polG2(true), polVS2Y(t)], [polG4(true), polVS4Y(t)],
            [polP3(t), polVBX(t)], [polP5(t), polVDX(t)]],
        Math.abs(polW1(t)));
}
