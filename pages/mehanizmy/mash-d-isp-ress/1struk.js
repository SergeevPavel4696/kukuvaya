...
import {
    ...
    polSBY, polSDY, polSS2X, polSS2Y, polSS4X, polSS4Y, polVerhKr
} from "./analiz.js";
import {polTekVr, polTokAB, polTokAD, polTokK, polTokOA, polTokTakt, polTokToch} from "./local_storage.js";
...
let contentTag =
    ...
    pol1_1([
        ...
        polPostrOtrezokNaTraektoriyu("postr_AD", "A", 5, "AD"),
        polPostrCentrMassNaOtrezke("postr_S4", "AD", 4),
...
const mehRis = elNaStr("meh_ris"), st1 = elNaStr("st_1"), st3 = elNaStr("st_3"), st5 = elNaStr("st_5"),
    shO = elNaStr("sh_O"), shA = elNaStr("sh_A"), shB = elNaStr("sh_B"), shD = elNaStr("sh_D"), zv1 = elNaStr("zv_OA"),
    zv2 = elNaStr("zv_AB"), zv3 = elNaStr("pol3"), zv4 = elNaStr("zv_AD"), zv5 = elNaStr("pol5"), s2 = elNaStr("S2"),
...
aktInterPu(mehRis, [
    ...
    ["gr_2", [st5, zv4, zv5, shA, shD, s4]],
    ["f-gr_0", [st1, zv1, shO]],
    ["f-gr_1", [st3, zv2, zv3, shA, shB, s2]],
    ["f-gr_2", [st5, zv4, zv5, shA, shD, s4]],
]);
...
const OA = polTokOA(), AB = polTokAB(), AD = polTokAD();
const planPol = elNaStr("plan-pol");


//Добавить план положений на страницу.
let planPolTag =
    "<g " + polTagPer(7 - polLevKr(), -32 - polVerhKr()) + ">" +
    polTagOpVr(null, null, null, null, null) +
    polTagOpSk(null, 60, 90, OA + AB, false, false, null, null) +
    polTagOpSk(null, -60, -90, OA + AD, false, false, null, null);
for (let i = 0; i < KOL; i++) {
    const t = i * TAKT / KOL;
    planPolTag +=
        ...
        polTagZvNeissl(polSAX(t), polSAY(t), polUgGrad(polFI4(t)), AD, 11) +
        polTagPolzNeissl(null, polSDY(t), -90) +
        polTagShNeissl(polSAX(t), polSAY(t)) +
        polTagShNeissl(0, polSBY(t)) +
        polTagShNeissl(0, polSDY(t)) +
...
planPolTag +=
    ...
    polTagZvAkt(polSAX(T), polSAY(T), polUgGrad(polFI4(T)), AD, 11) +
    polTagPolzAkt(null, polSDY(T), 90) +
    polTagShAkt(polSAX(T), polSAY(T)) +
    polTagShAkt(0, polSBY(T)) +
    polTagShAkt(0, polSDY(T)) +
    polTagCMAkt(polSS2X(T), polSS2Y(T)) +
    polTagCMAkt(polSS4X(T), polSS4Y(T));
planPol.innerHTML = planPolTag;
zadRisRazm(planPol, null, 14 + polPravKr() - polLevKr(), 64 + polVerhKr() - polNizhKr(), 0, 0);
