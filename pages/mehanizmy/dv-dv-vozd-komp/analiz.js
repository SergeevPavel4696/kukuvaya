...
import {
    polTokOA, polTokAB, polTokAS2, polTokOC, polTokCD, polTokCS4, polTokJs1, polTokJs2,
    polTokJs4, polTokJsDv, polTokK, polTokM1, polTokM2, polTokM3, polTokM4, polTokM5,
    polTokP3Max, polTokP5Max, polTokTakt, polTokToch, polTokW, polTokWDv, polTokD3, polTokD5
...
export function polFI4(t) {
    return Math.acos(-polSCX(t) / polTokCD());
}
...
export function polVerhKr() {
    return polMax([polTokOA() + polTokAB(), polTokOC() + polTokCD()]);
}
...
export function polP3(t) {
    ...
    if (polVBY(t) >= 0) {
        if (S < min + 0.3 * diap) {
            return P * (0.12 * (S - min) / (0.3 * diap));
        } else if (min + 0.3 * diap <= S && S < min + 0.4 * diap) {
            return P * (0.12 + 0.06 * (S - min - 0.3 * diap) / (0.1 * diap));
        } else if (min + 0.4 * diap <= S && S < min + 0.5 * diap) {
            return P * (0.18 + 0.09 * (S - min - 0.4 * diap) / (0.1 * diap));
        } else if (min + 0.5 * diap <= S && S < min + 0.6 * diap) {
            return P * (0.27 + 0.11 * (S - min - 0.5 * diap) / (0.1 * diap));
        } else if (min + 0.6 * diap <= S && S < min + 0.7 * diap) {
            return P * (0.38 + 0.17 * (S - min - 0.6 * diap) / (0.1 * diap));
        } else if (min + 0.7 * diap <= S && S < min + 0.8 * diap) {
            return P * (0.55 + 0.45 * (S - min - 0.7 * diap) / (0.1 * diap));
        } else {
            return P;
        }
    } else {
        if (S < min + 0.8 * diap) {
            ...
            return P * (0.3 * (S - min - 0.8 * diap) / (0.1 * diap));
        } else {
            return P * (0.3 + 0.7 * (S - min - 0.9 * diap) / (0.1 * diap));
        }
    }
}
...
export function polP5(t) {
    ...
    if (polVDY(t) >= 0) {
        if (S < min + 0.2 * diap) {
            return P * (0.3 + 0.04 * (S - min) / (0.2 * diap));
        } else if (min + 0.2 * diap <= S && S < min + 0.3 * diap) {
            return P * (0.34 + 0.03 * (S - min - 0.2 * diap) / (0.1 * diap));
        } else if (min + 0.3 * diap <= S && S < min + 0.4 * diap) {
            return P * (0.37 + 0.05 * (S - min - 0.3 * diap) / (0.1 * diap));
        } else if (min + 0.4 * diap <= S && S < min + 0.5 * diap) {
            return P * (0.42 + 0.08 * (S - min - 0.4 * diap) / (0.1 * diap));
        } else if (min + 0.5 * diap <= S && S < min + 0.6 * diap) {
            return P * (0.5 + 0.09 * (S - min - 0.5 * diap) / (0.1 * diap));
        } else if (min + 0.6 * diap <= S && S < min + 0.7 * diap) {
            return P * (0.59 + 0.11 * (S - min - 0.6 * diap) / (0.1 * diap));
        } else if (min + 0.7 * diap <= S && S < min + 0.8 * diap) {
            return P * (0.7 + 0.3 * (S - min - 0.7 * diap) / (0.1 * diap));
        } else {
            return P;
        }
    } else {
        if (S < min + 0.8 * diap) {
            ...
            return P * (0.3 + 0.24 * (S - min - 0.8 * diap) / (0.1 * diap));
        } else {
            return P * (0.54 + 0.46 * (S - min - 0.9 * diap) / (0.1 * diap));
        }
    }
}
...
export function polMomP5(t) {
    return -polP5(t) * polSCX(t) / 1000;
}

export function polMomFIn5(t) {
    return -polFIn5Y(t) * polSCX(t) / 1000;
}

export function polMomG5(t) {
    return -polG5(t) * polSCX(t) / 1000;
}
...
export function polMomR05(t) {
    return -polR05(t) * (polSDY(t) - polSCY(t)) / 1000;
}

export function polPlA(t) {
    return (polSDY(t) - polSCY(t)) / 1000;
}

export function polPlB(t) {
    return Math.abs(polSCX(t)) / 1000;
}
...
export function polMomG3(t) {
    return -polG3(t) * (polSAX(t) - polSBX(t)) / 1000;
}
...
export function polMomG2(t) {
    return -polG2(t) * (polSAX(t) - polSS2X(t)) / 1000;
}

export function polMomR03(t) {
    return -polR03(t) * (polSBY(t) - polSAY(t)) / 1000;
}
...
export function polUgP3VB(t) {
    return (polVBY(t) * polP3(t) >= 0 ? 0 : 180);
}

export function polUgP5VD(t) {
    return (polVDY(t) * polP5(t) >= 0 ? 0 : 180);
}
...
export function polMp(t) {
    return polMpMeh([[polG2(true), polVS2Y(t)], [polG3(true), polVBY(t)], [polG4(true), polVS4Y(t)],
        [polG5(true), polVDY(t)], [polP3(t), polVBY(t)], [polP5(t), polVDY(t)]], Math.abs(polW1(t)));
}
