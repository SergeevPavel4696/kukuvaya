...
import {
    polFI1, polFI2, polFI3, polFI4, polLev5, polLevKr, polNizhKr, polPravKr, polSAX, polSAY, polSBX, polSBXLevKr,
    polSBXPravKr, polSBY, polSBYLevKr, polSBYPravKr, polSDX, polSS2X, polSS2Y, polSS3X, polSS3XLevKr,
    polSS3XPravKr, polSS3Y, polSS3YLevKr, polSS3YPravKr, polSS4X, polSS4Y, polVerhKr
} from "./analiz.js";
import {
    polTekVr, polTokAB, polTokBC, polTokBD, polTokBS3, polTokK, polTokOA, polTokTakt, polTokToch, polTokX, polTokY
...
const mehRis = elNaStr("meh_ris"), st1 = elNaStr("st_1"), st3 = elNaStr("st_3"), st5 = elNaStr("st_5"),
    ...
    trD = elNaStr("tr_D"), x = elNaStr("X"), y = elNaStr("Y");
...
const Y = polTokY(), X = polTokX(), OA = polTokOA(), AB = polTokAB(), BC = polTokBC(), BS3 = polTokBS3(), BD = polTokBD();
...
let planPolTag =
    ...
    polTagOpSk(polLev5() - 90, Y, null, BD, false, 0, null, null);
for (let i = 0; i < KOL; i++) {
    const t = i * TAKT / KOL;
    planPolTag +=
        ...
        polTagZvNeissl(X, Y, polUgGrad(polFI3(t)), BC, 11) +
        polTagZvNeissl(polSBX(t), polSBY(t), polUgGrad(polFI4(t)), BD, 11) +
        polTagPolzNeissl(polSDX(t), Y, null) +
        polTagShNeissl(polSAX(t), polSAY(t)) +
        polTagShNeissl(polSBX(t), polSBY(t)) +
        polTagShNeissl(polSDX(t), Y) +
        polTagCMNeissl(polSS2X(t), polSS2Y(t)) +
        polTagCMNeissl(polSS3X(t), polSS3Y(t)) +
        polTagCMNeissl(polSS4X(t), polSS4Y(t));
}
planPolTag +=
    polTagRaspZv(null, null, null, X) +
    polTagRaspZv(X, null, 90, Y) +
    polTagTrOkr(0, 0, OA) +
    polTagTrDug(polSBXLevKr(), polSBYLevKr(), polSBXPravKr(), polSBYPravKr(), BC) +
    polTagTrCikl(TOCH, TAKT, polSS2X, polSS2Y) +
    polTagTrDug(polSS3XLevKr(), polSS3YLevKr(), polSS3XPravKr(), polSS3YPravKr(), BC - BS3) +
...
zadRisRazm(planPol, null, 39 + polPravKr(true) - polLevKr(), 14 + polVerhKr() - polNizhKr(), 0, 0);
